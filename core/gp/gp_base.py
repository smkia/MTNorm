...
class GP(object):
    ...
    def predict(self,hyperparams,Xstar=None,Xstar_r=None,**kwargs):
        ...
        predict on Xstar
        """
        if type(Xstar_r) is SP.ndarray: 
            Xstar = Xstar_r
        if type(Xstar) is SP.ndarray:
        ...
        #
        Kss_diag = self.covar.K(hyperparams['covar'], X=Xstar)
        v = LA.cho_solve((KV['L'], True), Kstar)
        Ystar_cov = Kss_diag - Kstar.T.dot(v)
        Ystar_cov = abs(Ystar_cov)
    ...
    def _is_cached(self,hyperparams,keys=None):
        ...
        for key in keys:
            for i in range(len(hyperparams[key])):
                if (self._covar_cache['hyperparams'][key][i]!=hyperparams[key][i]).any():
                    return False
        return True
